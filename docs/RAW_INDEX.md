# RAW INDEX

このファイルは Launch Lab リポジトリ内の主要ファイル Raw URL 一覧です。
別AIへ引き継ぐとき、必要に応じてここを参照してください。

## Root
- README  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/README.md

## docs
- AI ENTRYPOINT  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/AI_ENTRYPOINT.md

- RULES  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/RULES.md

- DECISIONS  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/DECISIONS.md

- BACKLOG  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/BACKLOG.md

- REQUIREMENTS  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/requirements.md

- SITEMAP  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/sitemap.md

- DISCORD STRUCTURE  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/discord-structure.md

- PRODUCT PLAN  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/product-plan.md

- CONTENT POLICY  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/docs/content-policy.md

## web
- web placeholder  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/web/.gitkeep

## previews
- previews placeholder  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/previews/.gitkeep

## assets
- assets placeholder  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/assets/.gitkeep

## community
- community placeholder  
  https://raw.githubusercontent.com/goroyattemiyo/launch-lab/main/community/.gitkeep

## 運用ルール
- 新しい重要ファイルを追加したら、この一覧も更新する
- 別AIへ渡すときは、まず `docs/AI_ENTRYPOINT.md` を渡す
- フルコンテキストが必要なときのみ、この一覧から追加で読む